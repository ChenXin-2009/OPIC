...
| 修正 `ecefToSolarSystem` 严格互逆 |  | 标记 @deprecated + 指向 CameraSynchronizer.syncFromCesium；旧实现保留兼容 |
...
| Worker 输出保留 TEME-like km，不做私有轴映射 |  | Worker 输出已正确（TEME-like km 格式）。sgp4Calculator.eciToThreeJS 在主线程做 (x,z,-y) 轴映射，SatelliteLayer 内部通过 eciSwappedToRenderWorld 合并处理。完整重构 Worker 协议需改 OrbitalInterpolator 类型，建议独立 PR |
...
| 与 satellite.js geodetic/ECF 一致到可视化误差 |  | 需运行时验证，无法在单元测试中完成 |
| ISS 经纬度与 ground track API 一致 |  | 需运行时验证 |
| 时间推进时无固定角偏差 |  | 需运行时验证 |
...
| Gaia/Hipparcos 数据加载层记录 epoch、parallax、proper motion |  | 项目无 Gaia/Hipparcos 加载代码；已在 `ExoplanetHostIndex` 类型中保留 `raDeg/decDeg/distancePc` 字段供宿主星使用 |
