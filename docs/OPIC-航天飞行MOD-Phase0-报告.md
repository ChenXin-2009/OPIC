...
**R2（RK4 定步长在 10,000× 时间加速下发散）已进一步缓解。** 后续实现已经在 Phase 1 Task 1.3 中加入"单帧子步数上限保护"策略，并通过 `test/verify-flight-dynamics.ts` 的 10,000× 时间加速回归验证：将大时间步拆分为多个物理子步，每个子步 ≤ 30s。
...
1. **Task 1.10**：Phase 1 端到端集成与交付验收
2. **Phase 2 / Task 2.1**：轨道根数双向转换边界工况补强（圆轨道、逆行、近抛物线等）
