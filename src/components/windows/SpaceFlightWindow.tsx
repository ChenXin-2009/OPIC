...
function BuilderTab({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* 部件目录 */}
      <div>
        <h3 className="text-xs font-semibold text-slate-400 uppercase mb-2">部件目录 ({allParts.length})</h3>
...
function PartRow({ part }: { part: RocketPart }) {
  return (
...
function MissionTab({
  ...
  const { telemetry, throttle, setThrottle, timeScale, setTimeScale, isRunning, launch, abort, separateStage } = sim;
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="space-y-1 text-xs">
          ...
          <StatusItem label="火箭网格" status="pending" detail="待 Task 1.6" />
          <StatusItem label="尾焰粒子" status="pending" detail="待 Task 1.6" />
          <StatusItem label="轨迹线" status="pending" detail="待 Task 1.6" />
          <StatusItem label="追踪相机" status="pending" detail="待 Task 1.7" />
        </div>
      </div>
